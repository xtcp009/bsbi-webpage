...
export function Header() {
  ...
  return (
    <header className="site-header relative z-50 overflow-x-clip text-ink">
      ...
      <div className="chrome-copy relative z-10 wrap flex items-center gap-3 py-3 lg:gap-6 lg:py-4">
        <Link
          href="/"
        ...
        <nav
          className="hidden min-w-0 flex-1 items-center justify-center gap-[clamp(1.25rem,2.6vw,2.75rem)] lg:flex"
          aria-label="Primary"
        >
          {nav.map((item) => {
        ...
        <div className="relative z-20 ml-auto flex shrink-0 items-center gap-2">
          <a
            href={site.phoneHref}
            className="inline-flex min-h-10 items-center gap-2 border border-line bg-[var(--header-ivory)] px-2.5 py-1.5 text-sm text-ink sm:min-h-11 sm:px-3"
          >
          ...
          <button
            type="button"
            className="inline-flex size-10 shrink-0 touch-manipulation items-center justify-center border border-line bg-[var(--header-ivory)] lg:hidden"
            aria-expanded={open}
      ...
      <nav
        id="mobile-nav"
        className={`relative z-10 border-t border-line/70 bg-[var(--header-ivory)] lg:hidden ${open ? "block" : "hidden"}`}
        aria-label="Mobile"
      >
