...
export function SocialLinks({ onDark = false, compact = false }: { onDark?: boolean; compact?: boolean }) {
  return (
    <nav aria-label="Social" className="flex flex-wrap items-center gap-2.5">
      {items.map((item) => (
        <HapticHit key={item.href} className="inline-flex rounded-full">
          <a
            href={item.href}
            target="_blank"
            ...
            className={`inline-flex items-center justify-center rounded-full border ${
              ...
              onDark
                ...
                : "border-line bg-cream/40 text-charleston hover:border-charleston"
            }`}
