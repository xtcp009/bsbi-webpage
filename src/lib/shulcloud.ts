...
export const SHULCLOUD_REVALIDATE_SECONDS = 120;
...
const ORIGINS = [
  process.env.SHULCLOUD_ORIGIN || site.shulcloudOrigin,
...
async function fetchHtml(path: string): Promise<{ html: string; url: string } | null> {
  ...
  for (const origin of ORIGINS) {
    try {
      ...
      if (!/shulcloud|today'?s calendar|ce_event_name|fck_widget_calendar/i.test(html)) continue;
...
> {
  ...
  const parshaMatch = homeHtml.match(/Parshat\s+([^<]+)/i);
...
export async function getShulcloudSnapshot(): Promise<ShulcloudSnapshot> {
  ...
  const parsed = parseShulcloudPages(home?.html ?? "", calendarPage?.html ?? "");
